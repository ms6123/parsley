...
private [machine] object XAssert {
    final inline def ensureRegularInstruction(inline ctx: =>Context): Unit = ctx match {
    ...
    final inline def ensureHandlerInstruction(inline ctx: =>Context): Unit = ctx match {
        case ctx: InterpreterContext =>
